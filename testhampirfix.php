...
$conn = new mysqli($servername, $username, $password, $dbname);
    ...
    $sqlSelect = "SELECT * from tb_kelas_pertama";
    $sqlSelect1 = "SELECT * from tb_kelas_kedua";
...
if (!$mysqli->query("DROP PROCEDURE IF EXISTS get_date") ||
    !$mysqli->query('CREATE PROCEDURE get_date() 
...
if (!$mysqli->query("DROP PROCEDURE IF EXISTS rekap") ||
    !$mysqli->query('CREATE PROCEDURE rekap() 
...
if (!$mysqli->query("DROP PROCEDURE IF EXISTS get_username") ||
    ...
    TRUNCATE TABLE tb_kelas_pertama;
	date_loop: LOOP
		...
		INSERT INTO tb_kelas_pertama (username, `time`, `waktu`, waktu1)
...
if (!$mysqli->query("DROP PROCEDURE IF EXISTS get_username1") ||
	...
	date_loop: LOOP
		...
		IF finished = 1 THEN
			LEAVE date_loop;
		END IF;
		INSERT INTO tb_kelas_kedua (username, `time`, `waktu`, waktu1)
		SELECT username,`day`, durations, rekap.`finished`
		FROM `rekap` WHERE durations >= cek_master AND rekap.`finished`<=cek_master1;
	END LOOP;
    CLOSE cur_master;
